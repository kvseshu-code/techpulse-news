...
function policy(k){
 ...
 const paras=String(d[k][1]).split(/\n\s*\n/).map(x=>x.trim()).filter(Boolean);$("#modalBody").innerHTML=`<div class="policy-content"><span class="eyebrow">TECHPULSE POLICY</span><h2>${esc(d[k][0])}</h2>${paras.map(x=>`<p>${esc(x)}</p>`).join("")}</div>`;show();
}
